...
// Define the structure for our AI response data
...
interface RiskAssessmentData {
  type: 'delay' | 'damage';
  summary: string; // The text paragraph from the AI
...
const ContractBuilder = () => {
  ...
  // State to hold the structured risk data from the AI
  ...
  const handleAiRiskAssessment = async () => {
    ...
    setAiRiskAssessment(null);
    try {
      const assessmentType = policyType === 'shipper' ? 'delay' : 'damage';

      const { data, error } = await supabase.functions.invoke('ai-risk-assessor', {
        body: {
          origin: sourceLocation,
          destination: destinationLocation,
          policyType: assessmentType,
        },
      });
      ...
      toast({
        ...
        description: error.message || "Could not generate risk assessment. Please try again.",
        variant: "destructive",
      });
    } finally {
                ...
                <CardContent className="space-y-4">
                  ...
                  {aiRiskAssessment && (
                      <div className="bg-[#1E3A5F] p-3 rounded-lg border border-[#D4AF37]/30 transition-all duration-500">
                        ...
                        <p className="text-xs text-[#CCD6F6]/90 font-serif mb-3 italic">
                          "{aiRiskAssessment.summary}"
                        </p>
                        ...
                        <div className="space-y-2">
                          {aiRiskAssessment.probabilities.map((p) => (
                              <div key={p.threshold} className="flex justify-between items-center font-serif text-sm">
                          <span className="text-[#CCD6F6]">
                           {`Prob. of > ${p.threshold}${aiRiskAssessment.type === 'delay' ? 'h delay' : '% damage'}:`}
                          </span>
                                <span className="text-lg font-semibold text-[#64FFDA]">
